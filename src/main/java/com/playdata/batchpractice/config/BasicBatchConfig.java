...
public class BasicBatchConfig {
    ...
    public JdbcBatchItemWriter<User> userDbWriter() {
        return new JdbcBatchItemWriterBuilder<User>()
                // User 객체의 필드를 보고 자동으로 SQL 파라미터로 매핑해 주는 설정
                ...
                .dataSource(dataSource) // 데이터베이스 정보
                .build();
    }
    ...
    public Step csvToDbStep() {
        return new StepBuilder("csvToDbStep", jobRepository)
                ...
                // chunk(10): 10개씩 묶어서 처리해!, 단위별로 작업 후 commit, 문제가 있다면 rollback
                // 단위를 나눠놓지 않으면 전체 데이터가 rollback 되기 때문에, 작은 단위로 나누어 진행
